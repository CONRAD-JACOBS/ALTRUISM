...
def _fit_nb_pvalue(df, term, alpha_nb):
    try:
        X = df[["Liking", "Robomentism", "Liking x Robomentism"]]
...
def run_power_nb(
    ...
    for N in N_values:
        k_nb = {"Liking": 0, "Robomentism": 0, "Liking x Robomentism": 0}
        mean_completed = []
        median_completed = []
        mean_mu_true = []
        ...
        for _ in range(nsim):
            ...
            for term in ("Liking", "Robomentism", "Liking x Robomentism"):
    ...
    with open(txt_path, "w") as f:
        ...
        f.write(
            df_out.pivot_table(
                index=["N"], columns=["term"], values=["power_nb"]
            ).to_string()
        )
        f.write("\n")
    ...
    for term in ("Liking", "Robomentism", "Liking x Robomentism"):
        ...
        plt.ylabel("Power")
        plt.title(
            f"Negative Binomial Power Curve for {term}"
