...
def run_power_nb(
    ...
    if b0_nb_values is None:
        b0_nb_values = (b0_nb,)
    else:
        b0_nb_values = tuple(b0_nb_values)
        if not b0_nb_values:
            raise ValueError("b0_nb_values must contain at least one intercept")

    for b0_value in b0_nb_values:
        for N in N_values:
            ...
            for _ in range(nsim):
                ...
                median_completed.append(df["completed"].median())
    ...
    with open(txt_path, "w") as f:
        ...
        f.write("NB intercept b0_nb_values: {}\n".format(list(b0_nb_values)))
        f.write("NB dispersion alpha_nb: {}\n".format(alpha_nb))
        f.write("betas (LIKE, MENT, INT): {}, {}, {}\n".format(b_like, b_ment, b_int))
        f.write("\nOutcome summaries by intercept and N:\n")
        f.write(
            df_out.groupby(["b0_nb", "N"])[
                ["mean_completed", "median_completed", "mean_mu_true"]
            ].mean().to_string()
        )
        f.write("\n\nPower table (NB):\n")
        f.write(
            df_out.pivot_table(
                index=["b0_nb", "N"], columns=["term"], values=["power_nb"]
            ).to_string()
        )
        f.write("\n")
    ...
    for term in ("LIKE", "MENT", "LIKExMENT"):
        ...
        for b0_value in b0_nb_values:
            d = df_out[
                (df_out["term"] == term) & (df_out["b0_nb"] == b0_value)
            ].sort_values("N")
            x = d["N"].to_numpy()
            ...
            line, = plt.plot(x, y, marker="o", label="b0_nb={}".format(b0_value))
            plt.fill_between(
                x,
                y_lo,
                y_hi,
